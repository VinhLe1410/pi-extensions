import {
  getMarkdownTheme,
  type Theme,
  type ThemeColor,
  UserMessageComponent,
} from "@earendil-works/pi-coding-agent";
import {
  Markdown,
  type MarkdownTheme,
  truncateToWidth,
  visibleWidth,
} from "@earendil-works/pi-tui";

const OSC133_ZONE_START = "\x1b]133;A\x07";
const OSC133_ZONE_END = "\x1b]133;B\x07";
const OSC133_ZONE_FINAL = "\x1b]133;C\x07";

const RAIL_GAP = " ";

type RenderFn = (width: number) => string[];
type Cleanup = () => void;

type PatchableUserMessagePrototype = {
  render: RenderFn;
  children?: unknown[];
  __promptUiUserMessageOriginalRender?: RenderFn;
  __promptUiUserMessagePatched?: boolean;
  __promptUiUserMessageWrapper?: RenderFn;
  __promptUiUserMessageActive?: boolean;
  __promptUiUserMessageGetTheme?: () => Theme | undefined;
};

type MarkdownLike = {
  text?: unknown;
};

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

function findMarkdownText(value: unknown): string | undefined {
  if (isRecord(value) && typeof (value as MarkdownLike).text === "string") {
    return (value as { text: string }).text;
  }

  if (!isRecord(value)) return undefined;

  const children = Array.isArray(value.children) ? value.children : [];
  for (const child of children) {
    const text = findMarkdownText(child);
    if (text !== undefined) return text;
  }

  return undefined;
}

function themeFg(theme: Theme, color: ThemeColor, text: string): string {
  try {
    return theme.fg(color, text);
  } catch {
    return text;
  }
}

function makeMarkdownTheme(theme: Theme): MarkdownTheme {
  return {
    ...getMarkdownTheme(),
    heading: (text) => themeFg(theme, "mdHeading", text),
    link: (text) => themeFg(theme, "mdLink", text),
    linkUrl: (text) => themeFg(theme, "mdLinkUrl", text),
    code: (text) => themeFg(theme, "mdCode", text),
    codeBlock: (text) => themeFg(theme, "mdCodeBlock", text),
    codeBlockBorder: (text) => themeFg(theme, "mdCodeBlockBorder", text),
    quote: (text) => themeFg(theme, "mdQuote", text),
    quoteBorder: (text) => themeFg(theme, "mdQuoteBorder", text),
    hr: (text) => themeFg(theme, "mdHr", text),
    listBullet: (text) => themeFg(theme, "mdListBullet", text),
    bold: (text) => theme.bold(text),
    italic: (text) => theme.italic(text),
    underline: (text) => theme.underline(text),
    strikethrough: (text) => theme.strikethrough(text),
  };
}

function fillLine(content: string, width: number): string {
  const truncated = truncateToWidth(content, Math.max(0, width), "");
  const pad = " ".repeat(Math.max(0, width - visibleWidth(truncated)));
  return `${truncated}${pad}`;
}

function renderPromptBoxLine(line: string, width: number, theme: Theme): string {
  if (width <= 0) return "";

  const rail = themeFg(theme, "accent", "│") + RAIL_GAP;
  const railWidth = visibleWidth(rail);
  if (width <= railWidth) return truncateToWidth(rail, width, "");

  const contentWidth = width - railWidth;
  return truncateToWidth(`${rail}${fillLine(line, contentWidth)}`, width, "");
}

export function renderUserMessagePromptBox(
  text: string,
  width: number,
  theme: Theme,
): string[] {
  if (width <= 0) return [""];

  const railWidth = visibleWidth(themeFg(theme, "accent", "│") + RAIL_GAP);
  const contentWidth = Math.max(1, width - railWidth);
  const renderer = new Markdown(
    text,
    0,
    0,
    makeMarkdownTheme(theme),
    { color: (content) => themeFg(theme, "userMessageText", content) },
    { preserveOrderedListMarkers: true },
  );
  const body = renderer.render(contentWidth);
  const contentLines = body.length > 0 ? body : [""];
  const border = themeFg(theme, "borderMuted", "─".repeat(width));

  return [
    truncateToWidth(border, width, ""),
    renderPromptBoxLine("", width, theme),
    ...contentLines.map((line) => renderPromptBoxLine(line, width, theme)),
    renderPromptBoxLine("", width, theme),
    truncateToWidth(border, width, ""),
  ];
}

export function installUserMessageStyle(getTheme: () => Theme | undefined): Cleanup {
  const prototype = UserMessageComponent.prototype as unknown as PatchableUserMessagePrototype;
  prototype.__promptUiUserMessageGetTheme = getTheme;
  prototype.__promptUiUserMessageActive = true;

  if (
    prototype.__promptUiUserMessagePatched &&
    prototype.render === prototype.__promptUiUserMessageWrapper
  ) {
    return () => {
      prototype.__promptUiUserMessageActive = false;
    };
  }

  prototype.__promptUiUserMessageOriginalRender = prototype.render;
  const wrapper = function renderWithPromptUiUserMessage(
    this: unknown,
    width: number,
  ): string[] {
    const original = prototype.__promptUiUserMessageOriginalRender ?? prototype.render;
    if (!prototype.__promptUiUserMessageActive) return original.call(this, width);

    const theme = prototype.__promptUiUserMessageGetTheme?.();
    if (!theme) return original.call(this, width);

    const text = findMarkdownText(this);
    if (text === undefined) return original.call(this, width);

    const lines = renderUserMessagePromptBox(text, width, theme);
    if (lines.length === 0) return lines;

    lines[0] = OSC133_ZONE_START + lines[0];
    lines[lines.length - 1] =
      OSC133_ZONE_END + OSC133_ZONE_FINAL + lines[lines.length - 1];
    return lines;
  };

  prototype.__promptUiUserMessageWrapper = wrapper;
  prototype.render = wrapper;
  prototype.__promptUiUserMessagePatched = true;

  return () => {
    prototype.__promptUiUserMessageActive = false;
  };
}
