...
async function runAnalyze(mctx: MenuContext): Promise<void> {
  ...
  mctx.syncState();
  mctx.ui.notify('GitNexus: analyzing codebase in index-only mode, this may take a while…', 'info');
...
function showHelp(mctx: MenuContext): void {
  mctx.ui.notify(
    ...
    '  /gitnexus analyze     — build/rebuild graph only (skips AGENTS.md/CLAUDE.md/skills)\n' +
