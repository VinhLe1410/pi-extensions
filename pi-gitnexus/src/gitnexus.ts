...
/**
 * Run `gitnexus analyze --index-only` and return the exit code (null on spawn error).
 *
 * `--index-only` builds the `.gitnexus/` graph while skipping GitNexus AI-context
 * file injection (`AGENTS.md`, `CLAUDE.md`, and `.claude/skills/`). Stops the MCP
 * process before reindexing so it does not query a half-rebuilt graph, and again
 * after completion so the next tool call respawns against the fresh index. If the
 * first run fails because the directory is not a git repository, retries once
 * with `--skip-git`.
 */
export async function runGitNexusAnalyze(cwd: string): Promise<number | null> {
  ...
  const analyzeArgs = ['--index-only'];
  let result = await runOnce(analyzeArgs);
  if (result.code !== 0 && /not a git repository/i.test(result.stderr)) {
    result = await runOnce([...analyzeArgs, '--skip-git']);
  }
