...
export default function(pi: ExtensionAPI) {
  ...
  pi.on('before_agent_start', async (event: { systemPrompt?: string }, ctx: ExtensionContext) => {
    ...
    const augmentLine = augmentEnabled
      ? 'Graph context will appear after search results. '
      : 'Auto-augment is off; use /gitnexus on to append graph context to search results. ';
    return {
      systemPrompt:
        event.systemPrompt +
        '\n\n[GitNexus active] ' + augmentLine +
    ...
    handler: async (args: string, ctx: ExtensionContext) => {
      ...
      if (sub === 'help') {
        ctx.ui.notify(
          ...
          '  /gitnexus analyze     — index code only (skips AGENTS.md/CLAUDE.md/skills)\n' +
      ...
      if (sub === 'analyze') {
        ...
        ctx.ui.notify('GitNexus: analyzing codebase in index-only mode, this may take a while…', 'info');
