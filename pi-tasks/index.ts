 ...
 *   TaskExecute  — Execute tasks as subagents (requires pi-subagents)
 *
 * Commands:
...
import { loadTasksConfig } from "./tasks-config.ts";
import { openSettingsMenu } from "./ui/settings-menu.ts";
...
export default function (pi: ExtensionAPI) {
  ...
  /** Spawn a subagent via pi.events RPC (requires pi-subagents extension). */
  ...
  /** Stop a subagent via pi.events RPC (requires pi-subagents extension). */
  ...
  function checkSubagentsVersion() {
    ...
    const unsub = pi.events.on(`subagents:rpc:ping:reply:${requestId}`, (raw: unknown) => {
      ...
      if (remoteVersion === undefined) {
        pendingWarning =
          "pi-subagents is outdated — please update for task execution support.";
        ...
        pendingWarning =
          `pi-subagents is outdated (protocol v${remoteVersion}, ` +
